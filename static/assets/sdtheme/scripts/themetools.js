...
var ScriptTool = function () {
    ...
    showenable = function (val) {
        if (val === 1 || val === "1") {
            ...
            return '<label class="label label-default label-sm"><i class="fa fa-ban"></i> 禁用</label>';
        } else if (val === -1 || val === "-1")
            return '<label class="label label-danger label-sm"><i class="fa fa-ban"></i> 删除</label>';
        else {
            return "";
        }
    }
    ...
    showenum = function (value, texts, css) {
